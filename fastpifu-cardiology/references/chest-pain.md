# Chest pain (suspected cardiac origin)

<!-- concept_ids: (completed by data team) -->
<!-- author: GIRFT OP guide module 2 + GIRFT chest pain pathway | reviewed_by: <lead> | last_updated: 2026-06 -->

## Scope

New presentation of chest pain with suspected cardiac origin, assessed via a Rapid Access Chest
Pain Clinic (RACPC), often ACP-led, with ECG, CTCA or functional imaging. Disposition depends on
risk and whether intervention is needed.

## PIFU — suitable for patient-initiated follow-up

- Criterion: Assessment complete, no high-risk features, on stable anti-anginal/secondary
  prevention. Supported when: investigations concluded and management plan settled.
- Criterion: Post-PCI outcome assessment can be done virtually. Supported when: symptoms reviewed
  remotely and recovery uncomplicated.
- Criterion: Symptoms requiring reassessment can be delivered via (nurse-led) PIFU/SPoA.

## RETAIN_FOLLOW_UP — timed specialist follow-up still needed

- High-risk features — coronary angiogram ± invasive imaging/physiological assessment and MDT
  discussion as appropriate.
- Anti-anginal medication being initiated/titrated, or secondary prevention not yet optimised.
- Pending investigations (CTCA, functional imaging) that would change management.

## DISCHARGE — no ongoing specialist follow-up needed

- Non-cardiac cause established and no ongoing specialist role required.

## Borderline / uncertain (resolve to RETAIN_FOLLOW_UP, lower confidence)

- Awaiting CTCA/functional imaging result.
- Anti-anginal therapy started but response not yet reviewed.

## Data that must be present to assess

| Required data point | Why it matters |
|---|---|
| Risk stratification / RACPC outcome | High-risk features exclude PIFU |
| Investigation results (CTCA / functional imaging) | Pending results mean RETAIN/INSUFFICIENT_DATA |
| Anti-anginal/secondary-prevention status | Active titration excludes PIFU |

## Surveillance / review interval

PIFU should ideally be limited to 6–12 months; clinical triage advisable where PIFU is requested.

## Source

GIRFT chest pain pathway; GIRFT Outpatient Operational Guide module 2 (June 2026).
