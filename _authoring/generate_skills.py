...
description: >
  ...
  confidence score and explicit reasoning. Use this whenever you need to screen a {specialty_lc}
  patient for PIFU, decide whether a patient can move to patient-initiated follow-up, be
  discharged, or must stay in timed follow-up, or process a {specialty_lc} clinic/operation letter
  for follow-up disposition. Covers {cond_blurb}. Reach for this skill even when the request
  doesn't say "PIFU" explicitly but asks what should happen next with a {specialty_lc} patient's
  follow-up.
---

# FastPIFU — {Specialty}

This skill decides what should happen next with a {specialty_lc} patient who is **already in
...
read all first. If none is provided, ask for the letter. Confirm this is a {specialty_lc} document;
...
def render_skill(spec):
    ...
    rules = ""
    if spec.get("rules"):
        rules = "\n**" + spec["Specialty"] + "-specific hard rules:**\n" + "\n".join(f"- {r}" for r in spec["rules"])
